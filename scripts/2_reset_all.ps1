...
foreach ($H in $HOSTS) {
    ...
    if ($H -eq "devices") {
        ...
        ssh $H "kubectl delete --all deployments,services,pods,configmaps -n iot-edge 2>/dev/null || true"
    }
