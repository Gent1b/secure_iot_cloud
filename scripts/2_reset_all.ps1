...
Usage:
    ...
    .\scripts\2_reset_all.ps1 -Hosts cloud,devices
#>

[CmdletBinding()]
param(
        [string[]]$Hosts = @("mqtt", "monitor", "cloud", "devices"),
        [string]$RepoDir = "secure_iot_cloud"
)
...
foreach ($H in $Hosts) {
    ...
    if ($H -eq "devices") {
        ...
        ssh $H "kubectl delete --all deployments,services,pods,configmaps -n iot-edge 2>/dev/null || true; kubectl delete --all secrets -n iot-edge 2>/dev/null || true"
    }
